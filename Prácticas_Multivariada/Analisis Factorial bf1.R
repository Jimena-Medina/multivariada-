...
ggcorrplot(R,type="lower",hc.order=TRUE , colors = c("green", "purple", "blue"))
...
Encontrar la proporcion varianza explicada. Se interpreta de tal forma que el
numero cercanos a 1, el factor explica mejor la variable.
...
La unicidad es el cuadrado del coeficiente del factor unico y se expresa como 
la porcion de la varianza explicada por el factor unico. Quiere decir que no se
puede explicar por otros.
...
bi_mod<-function(tipo){
  biplot.psych(fa(x1,nfactors = 2,fm="minres",rotate = tipo),col = c(4,5,6),pch = c(21,18),group = bfi[,"gender"])
}
sapply(rot,bi_mod)
